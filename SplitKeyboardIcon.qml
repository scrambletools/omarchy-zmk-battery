...
Item {
  ...
  Canvas {
    ...
    readonly property var stagger: [0.5, 0.25, 0.0, 0.15, 0.35]
    ...
    onPaint: {
      ...
      for (var half = 0; half < 2; half++) {
        ...
        var ty = y0 + innerStagger + key * rows + key * 0.05
        var pivotX = half === 0 ? hx + key * cols - thumb * 2 + thumbInset : hx + thumb * 2 - thumbInset
        ctx.save()
